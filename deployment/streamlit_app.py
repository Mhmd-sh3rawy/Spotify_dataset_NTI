...
def is_binary_feature(feature_name):
    """Check if a feature should be treated as binary based on its name"""
    binary_keywords = ['explicit', 'mode', 'key', 'time_signature', 'class']
    feature_lower = feature_name.lower()

    # Check for track_genre_ prefix
    if feature_lower.startswith('track_genre_'):
        return True

    # Check for other binary keywords
    return any(keyword in feature_lower for keyword in binary_keywords)
    ...
    with col1:
        ...
        for feature in feature_names[:mid_point]:
            if is_binary_feature(feature):
                input_data[feature] = float(st.selectbox(
                    f"{feature.replace('_', ' ').title()}",
                    options=[0, 1],
                    index=0,
                    help=f"Select binary value for {feature}"
                ))
            else:
...
def random_sample_interface():
    ...
    if st.button("🎲 Generate Random Song", type="primary"):
        ...
        for feature in feature_names:
            if is_binary_feature(feature):
                random_data[feature] = float(np.random.choice([0, 1]))
            else:
